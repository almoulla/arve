...
def compute_vpsd(star, label):

    # RV time series
    time, rv, rv_err, time_unit, rv_unit = [star.arve.data.rv[label][var] for var in ["time", "rv", "rv_err", "time_unit", "rv_unit"]]

    # compute power spectrum
    f, ps, phi, win_f, win_ps, win_area = periodogram(time, rv, rv_err, normalize=False, win_func=True)
...
def periodogram(t, y, sig=0, ofac=1, normalize=True, win_func=False):
    ...
    f, ps, phi = GLS(t, y, sig, ofac, normalize)
    ...
    if win_func:
        ...
        win_y = np.sin(2 * np.pi * fc * t)

        # power spectrum of window function
        win_f, win_ps, _ = GLS(t, win_y, sig, ofac)
